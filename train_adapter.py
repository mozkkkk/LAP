...
def get_args_parser():
    ...
    parser.add_argument('--output_dir', default='./runs_adapter',
                        help='path where to save, empty for no saving')
    parser.add_argument('--checkpoints_dir', default='./runs_adapter',
                        help='path where to save checkpoints, empty for no saving')
    parser.add_argument('--tensorboard_dir', default='./runs_adapter',
                        help='path where to save, empty for no saving')
...
def main(args):
    ...
    model, criterion = build_model(args, adapter=True, training=True)
    # move to GPU
    model.to(device)
    criterion.to(device)
   ...
   # lr_scheduler = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(optimizer, T_0=50, T_mult=2, eta_min=1e-6)
    ...
    if args.resume:
        ...
        # 只允许新模块的参数可训练
        for param in model_without_ddp.regression1.ad1.parameters():
            param.requires_grad = True
        for param in model_without_ddp.regression1.ad2.parameters():
            param.requires_grad = True
        param_dicts = [
            {"params": model_without_ddp.regression1.ad1.parameters()},
            {"params": model_without_ddp.regression1.ad2.parameters()},
            #{"params": model_without_ddp.classification1.ad1.parameters()},
            #{"params": model_without_ddp.fpn.ad1.parameters()},
            #{"params": model_without_ddp.fpn.ad2.parameters()},
            #{"params": model_without_ddp.ad1.parameters()},
            #{"params": model_without_ddp.adapter.parameters()},
        ]
        # Adam is used by default
        optimizer = torch.optim.Adam(param_dicts, lr=args.lr)
        #optimizer = torch.optim.AdamW(param_dicts, lr=args.lr)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if epoch % args.eval_freq == 0 and epoch != 0:
            ...
            print("dis1:", result[2], "dis2:", result[3], "best dis1:", np.min(dis1), )
            with open(run_log_name, "a") as log_file:
